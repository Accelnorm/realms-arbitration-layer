...
import { SAFE_POLICY_PAYOUT_COUNT_OFFSET } from '@utils/instructions/DisputeSafe/pdas'
...
const loadDisputeSafeAccounts = async (
  connection: ConnectionContext,
  governancesArray: GovernanceProgramAccountWithNativeTreasuryAddress[],
): Promise<AccountTypeDisputeSafe[]> => {
  const safePolicyDiscriminator = bs58.encode(
    DISPUTE_SAFE_ACCOUNT_DISCRIMINATORS.safePolicy,
  )

  const discovered = await Promise.all(
    governancesArray.map(async (governance) => {
      const safePolicies = await getFilteredProgramAccounts(
        connection.current,
        DISPUTE_SAFE_PROGRAM_ID,
        [
          {
            memcmp: {
              offset: 0,
              bytes: safePolicyDiscriminator,
            },
          },
          {
            memcmp: {
              offset: safePolicyAuthorityOffset,
              bytes: governance.pubkey.toBase58(),
            },
          },
        ],
      )

      return safePolicies.map(({ publicKey, accountInfo }) => {
        const authority = new PublicKey(accountInfo.data.slice(8, 40))
        const resolver = new PublicKey(accountInfo.data.slice(40, 72))
        ...
        return new AccountTypeDisputeSafe(governance, publicKey, {
          authority,
          resolver,
          payoutCount,
        })
...
const safePolicyAuthorityOffset = 8
...
const useGovernanceAssetsStore = create<GovernanceAssetsStore>((set, _get) => ({
  ...
  getGovernedAccounts: async (connection, realm) => {
    ...
    const [programAccounts, disputeSafeAccounts] = await Promise.all([
      getProgramAssetAccounts(connection, governancesWithNativeTreasuryAddress),
      loadDisputeSafeAccounts(connection, governancesWithNativeTreasuryAddress),
    ])
    accounts.push(...programAccounts)
    accounts.push(...disputeSafeAccounts)
    set((s) => {
...
const loadMintGovernanceAccounts = async (
  ...
  for (const index in possibleMintAccounts) {
    ...
    if (possibleMintAccount) {
      const data = possibleMintAccount.data as Buffer
...
> => {
  ...
  // 3 - Load accounts related to mint
  const [programAccounts, disputeSafeAccounts, governedTokenAccounts, mintAccounts] =
    await Promise.all([
      getProgramAssetAccounts(connection, governancesWithNativeTreasuryAddress),
      loadDisputeSafeAccounts(connection, governancesWithNativeTreasuryAddress),
