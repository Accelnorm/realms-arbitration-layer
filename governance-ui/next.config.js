...
config = withTM({
  webpack: (config, { isServer }) => {
    ...
    // Deduplicate @solana/buffer-layout to root copy so nested packages (e.g.
    // @civic/profile -> @solana/spl-token@0.3.7, @irys/web-upload-solana) all
    // share the same Layout class instance and instanceof checks pass.
    config.resolve.alias = {
      ...config.resolve.alias,
      '@solana/buffer-layout': require.resolve('@solana/buffer-layout'),
    }
    return config
  },
